...
Vec.prototype.move = function(dx,dy){// 自分 を 移動
    this.x += dx;
    this.y += dy;
}
...
function CircleEntity(x,y,radius,type,restitution,deceleration){
    ...
    this.collidedWithRect = function(r){ // 円 と 矩形 の 衝突 矩形 の ４ 辺 上 で 最も 円 に 近い 座標( nx, ny) を 求める
        var nx = Math.max( r.x, Math.min( this.x, r.x + r.w));
        var ny = Math.max( r.y, Math.min( this.y, r.y + r.h));
        if(!this.isHit( nx, ny)){// 衝突 なし → リターン
            return;
        }
        if(this.onhit){ // 衝突 時 の コール バック 　 　 　 　 　 　
            this.onhit(this,r);
        }
    ...
    this.collidedWithCircle = function(peer){
        ...
        if(this.type == BodyDynamic && peer.type == BodyStatic){
            ...
            peer.move(bNormUnit.x*overlap,bNormUnit*overlap);
